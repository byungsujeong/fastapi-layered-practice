...
class AnonymousBoardRepositoryImpl(AnonymousBoardRepository):
    ...
    def find_all(self) -> List[AnonymousBoard]:
        db = SessionLocal()
        try:
            query = db.query(AnonymousBoard).order_by(AnonymousBoard.created_at.desc())
            return query.all()
        finally:
            db.close()

    def find_by_id(self, board_id: str) -> AnonymousBoard | None:
        db = SessionLocal()
        try:
            query = db.query(AnonymousBoard).filter(AnonymousBoard.id == board_id)
